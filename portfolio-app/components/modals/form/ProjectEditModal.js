...
const ProjectEditModal = ({ isOpen, onClose, project }) => {
  ...
  // Initialiser les données du projet quand la modal s'ouvre
  useEffect(() => {
    if (isOpen && project) {
      setProjectData({
        title: project.title || '',
      ...
      setTempImageUrl(project.image_url || '/images/projects/default.jpg');
      setUploadError(null);
      setErrors({
        title: '',
        description: '',
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      console.log('Données du projet à mettre à jour (simulé) :', projectData);
      
      // Mettre à jour le projet dans l'état global
        ...
        <div className="flex flex-col h-[70vh]">
          <div className="flex-1 overflow-y-auto p-4 skills-scrollbar">
            ...
            <div className="mb-6">
              ...
              {errors.image_url && <p className={`${errorStyle} md:text-sm lg:text-base`}>{errors.image_url}</p>}
            </div>
